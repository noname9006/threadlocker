...
client.on(Events.ThreadCreate, async thread => {
  ...
  if (thread.parentId === CHANNEL_ID) {
    ...
    try {
      ...
      // Lock only the second latest thread and send a message
      const secondLatestThread = sortedThreads[1];
      if (secondLatestThread && !secondLatestThread.locked) {
        console.log(`Locking and sending message to the second latest thread: ${secondLatestThread.name}`);
        await secondLatestThread.send(MESSAGE_CONTENT);
        await secondLatestThread.setLocked(true);
        console.log(`Locked thread: ${secondLatestThread.name}`);
      }
