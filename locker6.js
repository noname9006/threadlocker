...
client.on(Events.ThreadCreate, async thread => {
  ...
  if (thread.parentId === CHANNEL_ID) {
    ...
    try {
      ...
      // Send a message to the second latest thread and then lock all previous threads if they are not already locked
      let messageSent = false;
      for (const existingThread of sortedThreads) {
        if (existingThread.id !== thread.id && existingThread.createdAt < thread.createdAt) {
          try {
            // Unarchive the thread if it is archived
            if (existingThread.archived) {
              console.log(`Unarchiving thread: ${existingThread.name}`);
            ...
            // Lock the thread if it is not already locked
